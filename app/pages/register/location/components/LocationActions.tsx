...
interface LocationActionsProps {
  location: LocationWithProperty;
  onEdit?: (location: LocationWithProperty) => void;
  onDelete?: (location: LocationWithProperty) => void;
}

export const LocationActions = memo<LocationActionsProps>(({ location, onEdit, onDelete }) => {
  ...
  return (
    <div className="flex items-center gap-x-6">
      <button
        ...
        title="Editar"
        aria-label={`Editar localização ${location.name}`}
      >
      ...
      <button
        ...
        title="Excluir"
        aria-label={`Excluir localização ${location.name}`}
      >
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path
            strokeLinecap="round"
