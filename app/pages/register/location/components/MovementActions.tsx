...
interface MovementActionsProps {
  ...
  onEdit: (movement: LocationMoviment) => void;
  onDelete: (movement: LocationMoviment) => void;
}

export const MovementActions = memo<MovementActionsProps>(({ movement, onEdit, onDelete }) => {
  ...
  return (
    <div className="flex items-center gap-x-6">
      <button
        ...
        title="Editar"
        aria-label={`Editar movimentação ${movement.type}`}
      >
      ...
      <button
        ...
        title="Excluir"
        aria-label={`Excluir movimentação ${movement.type}`}
      >
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path
            strokeLinecap="round"
...
MovementActions.displayName = 'MovementActions';
