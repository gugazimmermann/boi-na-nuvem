...
import { MovementAnimalsModal } from '../components/MovementAnimalsModal';

interface LocationEntryExitTabProps {
  location: Location;
  entryExitSearch: string;
  entryExitSort: { key: string; direction: 'asc' | 'desc' } | null;
  debouncedEntryExitSearch: string;
  onEntryExitSearchChange: (value: string) => void;
  onEntryExitSortChange: (sort: { key: string; direction: 'asc' | 'desc' } | null) => void;
}

export function LocationEntryExitTab({
  location,
  entryExitSearch,
  entryExitSort,
  debouncedEntryExitSearch,
  onEntryExitSearchChange,
  onEntryExitSortChange,
}: LocationEntryExitTabProps) {
  ...
  const [selectedMovement, setSelectedMovement] = useState<LocationMoviment | null>(null);
  ...
  const handleSubmitMovement = useCallback(
    async (formData: MovementFormData) => {
      try {
        ...
        console.log('Creating entry/exit movement:', {
          ...formData,
        ...
        console.error('Error creating entry/exit movement:', error);
      } finally {
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    try {
      ...
      console.log('Deleting entry/exit movement:', movementToDelete);
      ...
      console.error('Error deleting entry/exit movement:', error);
    } finally {
      setIsDeleting(false);
    }
  }, [movementToDelete]);
  ...
  // Animals modal handlers
  const handleShowAnimals = useCallback((movement: LocationMoviment) => {
    setSelectedMovement(movement);
    setIsAnimalsModalOpen(true);
  }, []);

  const handleCloseAnimalsModal = useCallback(() => {
    setIsAnimalsModalOpen(false);
    setSelectedMovement(null);
  }, []);

  // Helper function to get animal count for a movement
  const getAnimalCountForMovement = useCallback((movementId: string) => {
    return ANIMAL_LOCATIONS.filter((al) => al.locationMovimentId === movementId).length;
  }, []);

  // Handler to navigate to responsible person details
  const handleResponsibleClick = useCallback(
    (movement: LocationMoviment) => {
      // Store context that we're coming from location entry/exit tab
  ...
  // Filter movements for this specific location and include only ENTRY and EXIT types
  ...
  const filteredLocationMovements = useMemo(
    () => searchMovements(baseLocationMovements, debouncedEntryExitSearch),
    [baseLocationMovements, debouncedEntryExitSearch, searchMovements],
  );
  ...
  const sortedLocationMovements = useMemo(() => {
    ...
    return [...filteredLocationMovements].sort((a, b) => {
      let aValue: any = a[entryExitSort.key as keyof LocationMoviment];
      let bValue: any = b[entryExitSort.key as keyof LocationMoviment];

      // Handle date sorting
      if (entryExitSort.key === 'createdAt') {
        aValue = new Date(aValue).getTime();
        bValue = new Date(bValue).getTime();
      }

      // Handle responsible sorting (by name)
      if (entryExitSort.key === 'responsibleType') {
      ...
      if (aValue < bValue) return entryExitSort.direction === 'asc' ? -1 : 1;
      if (aValue > bValue) return entryExitSort.direction === 'asc' ? 1 : -1;
      return 0;
    });
  }, [filteredLocationMovements, entryExitSort, getResponsibleInfo]);
  ...
  const tableConfig: TableConfig<LocationMoviment> = {
    title: 'Entradas e Saídas da Localização',
    subtitle: undefined,
    count: sortedLocationMovements.length,
    countLabel: 'movimentações',
    actions: {
      add: {
        label: 'Adicionar Entrada/Saída',
    ...
    columns: [
      {
        ...
        onSort: () => {
          const newDirection =
            entryExitSort?.key === 'createdAt' && entryExitSort?.direction === 'asc'
              ? 'desc'
              : 'asc';
          onEntryExitSortChange({ key: 'createdAt', direction: newDirection });
        },
        render: (value: string) => (
      ...
      },
      {
        ...
        onSort: () => {
          const newDirection =
            entryExitSort?.key === 'type' && entryExitSort?.direction === 'asc' ? 'desc' : 'asc';
          onEntryExitSortChange({ key: 'type', direction: newDirection });
        },
      ...
      },
      {
        ...
        onSort: () => {
          const newDirection =
            entryExitSort?.key === 'quantity' && entryExitSort?.direction === 'asc'
              ? 'desc'
              : 'asc';
          onEntryExitSortChange({ key: 'quantity', direction: newDirection });
        },
        render: (value: number, movement: LocationMoviment) => {
          const animalCount = getAnimalCountForMovement(movement.id);
          return (
            <button
              onClick={() => handleShowAnimals(movement)}
      ...
      },
      {
        ...
        onSort: () => {
          const newDirection =
            entryExitSort?.key === 'responsibleType' && entryExitSort?.direction === 'asc'
              ? 'desc'
              : 'asc';
          onEntryExitSortChange({ key: 'responsibleType', direction: newDirection });
        },
    ...
    search: {
      placeholder: 'Pesquisar entradas e saídas...',
      value: entryExitSearch,
      onChange: onEntryExitSearchChange,
    },
    ...(hasPagination
      ? {
          pagination: {
      ...
      <>
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-6">
            Esta localização ainda não possui entradas ou saídas registradas.
          </p>
          <Button
            onClick={handleOpenDrawer}
            config={{
          ...
          >
            Adicionar Primeira Entrada/Saída
          </Button>
        </div>
        ...
        <Drawer
          ...
          data-testid="add-entry-exit-drawer"
        >
          <DrawerHeader title="Adicionar Entrada/Saída" onClose={handleCloseDrawer} />
          <DrawerBody>
            <AddMovementForm
              ref={formRef}
              location={location}
              ...
              allowedTypes={[LocationMovimentType.ENTRY, LocationMovimentType.EXIT]}
            />
          </DrawerBody>
          <DrawerFooter>
            <Button
  ...
  return (
    <>
      ...
      >
        <DrawerHeader
          title={editingMovement ? 'Editar Entrada/Saída' : 'Adicionar Entrada/Saída'}
          onClose={handleCloseDrawer}
        />
        <DrawerBody>
          <AddMovementForm
      ...
      <ConfirmationModal
        ...
        variant="error"
        loading={isDeleting}
        data-testid="delete-entry-exit-modal"
      />

      {/* Animals Modal */}
      <MovementAnimalsModal
        isOpen={isAnimalsModalOpen}
        onClose={handleCloseAnimalsModal}
