...
interface BuyerDetailsTabProps {
  buyer: Buyer;
}

export function BuyerDetailsTab({ buyer }: BuyerDetailsTabProps) {
  return (
        ...
        <dl className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-3">
            <InfoItem
              ...
              value={<span className="text-lg font-semibold cursor-pointer">{buyer.name}</span>}
        ...
        <dl className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-3">
            <InfoItem
              ...
              label="Endereço"
              value={
                [buyer.street, buyer.number, buyer.neighborhood].filter(Boolean).join(', ') ||
                'Não informado'
              }
              iconBgColor={INFO_ITEM_CONSTANTS.ICON_BG_COLORS.EMERALD}
              iconTextColor={INFO_ITEM_CONSTANTS.ICON_TEXT_COLORS.EMERALD}
              ...
              <InfoItem
                ...
                value={
                  <span className="font-mono bg-gray-50 dark:bg-gray-700 px-3 py-2 rounded-lg">
                    {buyer.latitude.toFixed(6)}, {buyer.longitude.toFixed(6)}
                  </span>
                }
            ...
            <div>
              ...
              <div className="rounded-xl overflow-hidden border-2 border-gray-200 dark:border-gray-600 shadow-lg">
                <MapWithFallback
                  ...
                  address={(() => {
                    const addressParts = [];
                    if (buyer.street) addressParts.push(buyer.street);
                    if (buyer.number) addressParts.push(buyer.number);
                    if (buyer.neighborhood) addressParts.push(buyer.neighborhood);
                    if (buyer.city) addressParts.push(buyer.city);
                    if (buyer.state) addressParts.push(buyer.state);
