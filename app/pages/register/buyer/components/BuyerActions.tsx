...
export const BuyerActions = memo<BuyerActionsProps>(({ buyer, onEdit, onDelete }) => {
  ...
  return (
    <div className="flex items-center gap-x-6">
      <button
        ...
        title="Editar"
        aria-label={`Editar comprador ${buyer.name}`}
      >
      ...
      <button
        ...
        title="Excluir"
        aria-label={`Excluir comprador ${buyer.name}`}
      >
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path
            strokeLinecap="round"
...
BuyerActions.displayName = 'BuyerActions';
