...
import { BuyerDashboardTab, BuyerDetailsTab, BuyerPropertiesTab } from './tabs';

export default function BuyerDetailPage() {
  const { id } = useParams();
  const { buyers, loading, error, refetch, deleteBuyer } = useBuyers();
  const { handleBack, handleEditBuyer } = useBuyerNavigation();
  ...
  const buyer = buyers?.find((buy) => buy.id === id);
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    try {
      ...
      // Navigate back to buyers list after successful deletion
      handleBack();
    } catch (error) {
      console.error('Erro ao excluir comprador:', error);
      // O erro já é tratado pelo hook useBuyers
    } finally {
      setIsDeleting(false);
    }
  }, [buyer, deleteBuyer, handleBack]);

  const handleCancelDelete = useCallback(() => {
    setShowDeleteModal(false);
  }, []);

  const handleEditBuyerClick = useCallback(() => {
    if (buyer) {
      handleEditBuyer(buyer.id, true);
    }
  }, [buyer, handleEditBuyer]);
  ...
  if (loading) {
    return (
      <LoadingState
        message="Carregando detalhes do comprador..."
  ...
  if (error) {
    return (
      <ErrorState
        error={error}
        onRetry={refetch}
        title="Erro ao carregar comprador"
        className={LAYOUT_CONSTANTS.CONTAINER_CLASSES}
      />
    );
  }
        ...
        <div className={`${LAYOUT_CONSTANTS.CONTENT_CLASSES} px-4 py-12`}>
          <div className="text-center">
            ...
            <p className="text-lg text-gray-600 dark:text-gray-400 mb-8 max-w-md mx-auto">
              O comprador solicitado não foi encontrado ou pode ter sido removido.
            </p>
            <button
              onClick={handleBack}
    ...
    <div className={LAYOUT_CONSTANTS.CONTAINER_CLASSES}>
      <div className={`${LAYOUT_CONSTANTS.CONTENT_CLASSES} px-4 py-6`}>
        ...
        <Header
          ...
          actions={<BuyerStatusBadge status={buyer.status} />}
        />
        ...
        <div className="tab-content mb-8">
          {activeTab === 'dashboard' && buyer && <BuyerDashboardTab buyer={buyer} />}
          {activeTab === 'details' && buyer && <BuyerDetailsTab buyer={buyer} />}
          {activeTab === 'properties' && buyer && (
            <BuyerPropertiesTab
              buyer={buyer}
        ...
        {/* Actions */}
        <Actions
          title="Ações do Comprador"
          description="Gerencie este comprador e suas configurações"
          actions={[
            {
              id: 'edit',
              label: 'Editar Comprador',
              onClick: handleEditBuyerClick,
              variant: 'warning',
              icon: (
        ...
        <ConfirmationModal
          ...
          message={
            buyer
              ? `Tem certeza que deseja excluir o comprador "${buyer.name}"? Esta ação não pode ser desfeita.`
              : ''
          }
          confirmText="Excluir"
          cancelText="Cancelar"
          variant="error"
          loading={isDeleting}
          data-testid="delete-buyer-modal"
        />
      </div>
    </div>
  );
