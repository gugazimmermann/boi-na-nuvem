        ...
        <dl className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <InfoItem
              ...
              label="Nome Completo"
              value={<span className="text-lg font-semibold cursor-pointer">{employee.name}</span>}
        ...
        <dl className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-3">
            <InfoItem
              ...
              label="Endereço"
              value={
                [employee.street, employee.number, employee.neighborhood]
                  .filter(Boolean)
                  .join(', ') || 'Não informado'
              }
              iconBgColor={INFO_ITEM_CONSTANTS.ICON_BG_COLORS.EMERALD}
              ...
              <InfoItem
                ...
                value={
                  <span className="font-mono bg-gray-50 dark:bg-gray-700 px-3 py-2 rounded-lg">
                    {employee.latitude.toFixed(6)}, {employee.longitude.toFixed(6)}
                  </span>
                }
            ...
            <div>
              ...
              <div className="rounded-xl overflow-hidden border-2 border-gray-200 dark:border-gray-600 shadow-lg">
                <MapWithFallback
                  ...
                  address={(() => {
                    const addressParts = [];
                    if (employee.street) addressParts.push(employee.street);
                    if (employee.number) addressParts.push(employee.number);
                    if (employee.neighborhood) addressParts.push(employee.neighborhood);
                    if (employee.city) addressParts.push(employee.city);
                    if (employee.state) addressParts.push(employee.state);
                    if (employee.country) addressParts.push(employee.country);
