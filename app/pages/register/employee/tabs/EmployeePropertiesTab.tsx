...
export function EmployeePropertiesTab({
  employee,
  propertiesSearch,
  propertiesSort,
  ...
  // Get properties associated with this employee
  const employeeProperties = useMemo(() => {
    const propertyIds = PROPERTYHASEMPLOYEE.filter((emp) => emp.employeeId === employee.id).map(
      (emp) => emp.propertyId,
    );
  ...
  const filteredProperties = useMemo(() => {
    ...
    return employeeProperties.filter((property) => {
  ...
  }, [employeeProperties, debouncedPropertiesSearch, getPropertyStatusInfo]);
  ...
  const tableConfig: TableConfig<Property> = {
    ...
    subtitle: `Colaborador: ${employee.name}`,
      ...
      <div className="text-center py-12">
        ...
        <p className="text-gray-600 dark:text-gray-400 max-w-md mx-auto">
          {debouncedPropertiesSearch.trim()
            ...
            : 'Este colaborador ainda não está associado a nenhuma propriedade.'}
        </p>
      </div>
    );
  }
