...
interface SupplierPropertiesTabProps {
  supplier: Supplier;
...
export function SupplierPropertiesTab({
  supplier,
  propertiesSearch,
  propertiesSort,
  ...
  // Get properties associated with this supplier
  const supplierProperties = useMemo(() => {
    const propertyIds = PROPERTYHASSUPPLIER.filter((rel) => rel.supplierId === supplier.id).map(
      (rel) => rel.propertyId,
    );
  ...
  const filteredProperties = useMemo(() => {
    ...
    return supplierProperties.filter((property) => {
  ...
  }, [supplierProperties, debouncedPropertiesSearch, getPropertyStatusInfo]);
  ...
  const tableConfig: TableConfig<Property> = {
    title: 'Propriedades Fornecidas',
    subtitle: `Fornecedor: ${supplier.name}`,
      ...
      <div className="text-center py-12">
        ...
        <p className="text-gray-600 dark:text-gray-400 max-w-md mx-auto">
          {debouncedPropertiesSearch.trim()
            ...
            : 'Este fornecedor ainda não está associado a nenhuma propriedade.'}
        </p>
      </div>
    );
  }
