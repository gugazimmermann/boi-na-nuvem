...
import { SUPPLIER_CONFIG } from '../config/supplierConfig';
...
export function useSupplierFilters({ suppliers }: UseSupplierFiltersProps) {
  ...
  const matchesSearchTerm = useCallback(
    (supplier: Supplier, term: string) => {
      const normalizedTerm = normalizeText(term);
      const searchFields = SUPPLIER_CONFIG.searchFields.map(
        (field) => supplier[field as keyof Supplier] || '',
      );
  ...
  const filteredSuppliers = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      filtered = filtered.filter((supplier) => matchesSearchTerm(supplier, searchTerm));
    }
    ...
    return filtered;
  }, [suppliers, searchTerm, statusFilter, sortConfig, matchesSearchTerm]);
