...
import { SERVICEPROVIDER_CONFIG } from '../config/serviceProviderConfig';
...
interface UseServiceProviderFiltersProps {
  serviceProviders: ServiceProvider[];
}

export function useServiceProviderFilters({ serviceProviders }: UseServiceProviderFiltersProps) {
  ...
  const matchesSearchTerm = useCallback(
    (serviceProvider: ServiceProvider, term: string) => {
      const normalizedTerm = normalizeText(term);
      const searchFields = SERVICEPROVIDER_CONFIG.searchFields.map(
        (field) => serviceProvider[field as keyof ServiceProvider] || '',
      );
  ...
  const filteredServiceProviders = useMemo(() => {
    ...
    if (statusFilter !== 'all') {
      filtered = filtered.filter((serviceProvider) => serviceProvider.status === statusFilter);
    }

    if (searchTerm.trim()) {
      filtered = filtered.filter((serviceProvider) =>
        matchesSearchTerm(serviceProvider, searchTerm),
      );
    }
    ...
    return filtered;
  }, [serviceProviders, searchTerm, statusFilter, sortConfig, matchesSearchTerm]);
