...
interface ServiceProviderDetailsTabProps {
  serviceProvider: ServiceProvider;
}

export function ServiceProviderDetailsTab({ serviceProvider }: ServiceProviderDetailsTabProps) {
  return (
        ...
        <dl className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-3">
            <InfoItem
              ...
              value={
                <span className="text-lg font-semibold cursor-pointer">{serviceProvider.name}</span>
              }
        ...
        <dl className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-3">
            <InfoItem
              ...
              label="Endereço"
              value={
                [serviceProvider.street, serviceProvider.number, serviceProvider.neighborhood]
                  .filter(Boolean)
                  .join(', ') || 'Não informado'
              }
              iconBgColor={INFO_ITEM_CONSTANTS.ICON_BG_COLORS.EMERALD}
            ...
            <InfoItem
              ...
              value={(() => {
                const parts = [];
                if (serviceProvider.city) parts.push(serviceProvider.city);
                if (serviceProvider.state) parts.push(serviceProvider.state);
                if (serviceProvider.country) parts.push(serviceProvider.country);
              ...
              <InfoItem
                ...
                value={
                  <span className="font-mono bg-gray-50 dark:bg-gray-700 px-3 py-2 rounded-lg">
                    {serviceProvider.latitude.toFixed(6)}, {serviceProvider.longitude.toFixed(6)}
                  </span>
                }
            ...
            <div>
              ...
              <div className="rounded-xl overflow-hidden border-2 border-gray-200 dark:border-gray-600 shadow-lg">
                <MapWithFallback
                  ...
                  address={(() => {
                    const addressParts = [];
                    if (serviceProvider.street) addressParts.push(serviceProvider.street);
                    if (serviceProvider.number) addressParts.push(serviceProvider.number);
                    if (serviceProvider.neighborhood)
                      addressParts.push(serviceProvider.neighborhood);
                    if (serviceProvider.city) addressParts.push(serviceProvider.city);
                    if (serviceProvider.state) addressParts.push(serviceProvider.state);
                    if (serviceProvider.country) addressParts.push(serviceProvider.country);
